...
vec3 generalLighting(LightColor color, vec3 lightDir, vec3 cameraDir){
    ...
    vec3 ambientLighting = color.ambient * snowDiffuse;

    float diffuseStrength = max(dot(lightDir, norm), 0.0f);
    vec3 diffuseLighting = color.diffuse * diffuseStrength * snowDiffuse;

    float specularStrength = pow(max(dot(reflectDir, cameraDir), 0.0f), snowShininess);
    vec3 specularLighting = color.specular * specularStrength * snowSpecular;

    return ambientLighting + diffuseLighting + specularLighting;
}

vec3 dirLightLighting(DirLight dl){
    return generalLighting(dl.color, -dl.direction, cameraPos - fIn.pos);
}

vec3 pointLightLighting(PointLight pl){
    vec3 resLighting = generalLighting(pl.color, pl.position - fIn.pos, cameraPos - fIn.pos);
...
vec3 spotLightLighting(SpotLight sl){
    ...
    vec3 resLighting = generalLighting(sl.color, lightDir, cameraPos - fIn.pos);
